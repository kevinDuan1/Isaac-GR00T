...
class FilteredRobotInferenceServer(RobotInferenceServer):
    def __init__(self, policy, port: int = 5555, api_token: str = None):
        super().__init__(policy, port=port, api_token=api_token)
        self.policy = policy
        self.register_endpoint("get_action", self._handle_filtered_action, requires_input=True)

    def _handle_filtered_action(self, observations: dict):
        raw_actions = self.policy.get_action(observations)
        filtered_actions = {}
        for key, value in raw_actions.items():
            print(f"Raw action key: {key}, shape: {value.shape}")
            if "action.torso" in key:
                continue
...
def main(args: ArgsConfig):
    if args.server:
        ...
        # Start the server
        server = FilteredRobotInferenceServer(policy, port=args.port, api_token=args.api_token)
